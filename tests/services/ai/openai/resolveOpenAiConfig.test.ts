...
describe('resolveOpenAiConfig', () => {
  ...
  it('should not inject metadata into prompts (metadata is added post-processing)', () => {
    // Arrange - createMockConfig has no title, authors, date
    const config = createMockConfig();
    const stepConfig = createStepConfig();

    // Act
    const handoutResult = resolveOpenAiConfig(config, 'handout', stepConfig);
    const summaryResult = resolveOpenAiConfig(config, 'summary', stepConfig);
    const systemPrompt = handoutResult.systemPrompt as { singlePass: string; incremental: string };

    // Assert - prompts do not contain metadata block (metadata is added post-processing)
    ...
    expect(summaryResult.systemPrompt).not.toContain('METADATA (use these values exactly)');
  });
